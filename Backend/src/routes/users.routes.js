...
router.get('/', authenticate, authorizeRoles('admin'), usersController.getAllUsers.bind(usersController));

// POST /users - Create a new user (admin or student)
// Restricted to admins to prevent privilege escalation
router.post('/', authenticate, authorizeRoles('admin'), usersController.createUser.bind(usersController));
